...
from eval.lerf_ovs import evalute, get_queries
...
def rendering(output_dir, views, gaussians, pipe, bg, 
    ...
    for idx, view in enumerate(tqdm(views, desc="Rendering progress")):
        ...
        for i, query in enumerate(target_text):
            ...
            binary_mask = image > threshold  ##TODO test threshold

            render_path = os.path.join(view_name, "rgb")  # Inside frame_name folder
            os.makedirs(render_path, exist_ok=True)                      
            torchvision.utils.save_image(image, os.path.join(render_path, f"{query}.png"))

            # Save mask for IoU compute
            mask_path = os.path.join(view_name, "mask")   # Inside frame_name folder
            ...
            query_path = os.path.join(view_name, "query") # Inside frame_name folder
...
def generate(dataset, opt, pipeline, mask_thresh, device="cuda"):
    ...
    with torch.no_grad():        
        ...
        gs_mask_pred = get_mask(semantics, pts, clip_model, mask_thresh)
...
if __name__ == "__main__":
    ...
    generate(model.extract(args), args.iteration, pipeline.extract(args), args.mask_thresh)
